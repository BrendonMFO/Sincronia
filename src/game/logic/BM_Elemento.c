...
void BM_Elemento_adicionar_mouse_listener()
{
  ...
  for (i = 0; i < 6; i++)
  {
    ...
    *id = i + 1;
    sourceW = bm_al_bitmap_width(SPRITES(BM_IMG_ELEMENTOS)->Imagem) / SPRITES(BM_IMG_ELEMENTOS)->imagem->columns;
    sourceH = bm_al_bitmap_height(SPRITES(BM_IMG_ELEMENTOS)->Imagem) / SPRITES(BM_IMG_ELEMENTOS)->imagem->lines;
